...
def case_root() -> Path:
    ...
    return get_hermes_home() / "dataviz-fix" / "cases"
...
def on_pre_llm_call(**kwargs: Any) -> dict[str, str] | None:
    ...
    skill_root = get_hermes_home() / "skills" / "data-science"
    case_manager = skill_root / "dataviz-fix" / "scripts" / "case_manager.py"
    return {
        "context": (
            "Hermes dataviz release guard is active for this chart-repair turn. "
            f"Run the dataviz-fix case manager at {str(case_manager)!r}, use "
            f"{str(skill_root)!r} as --skills-root, and pass exactly "
            f"--session {case_session!r} to start. Use a fresh leaf reviewer "
            "through delegate_task with file, terminal, and vision toolsets. "
            "The final response must attach the exact reviewed image with "
            "MEDIA:/absolute/path. Hermes will withhold it unless this "
            "turn-bound case reaches user_review with an independent Send "
            "verdict and the delivered file hash matches the reviewed artifact."
        )
    }
