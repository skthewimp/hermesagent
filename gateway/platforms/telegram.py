...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_phase1_voice_message(self, msg: Message, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            logger.info("[Telegram] Voice note received file_id=%s", file_id)
            if not _get_openai_api_key():
                await msg.reply_text(
                    "I received your voice note, but OPENAI_API_KEY is not configured.\n\n"
                    "I can't transcribe voice notes yet."
                )
                logger.warning("[Telegram] Voice note skipped: OPENAI_API_KEY missing")
                return
            ...
            await self.handle_message(event)
            logger.info("[Telegram] Forwarded transcribed voice note to message handler")
        except Exception as exc:
            logger.exception("[Telegram] Voice note handling failed")
            await msg.reply_text(
                "I received your voice note, but voice handling failed: "
                f"{exc}\n\n"
                "I couldn't forward it."
            )
        finally:
            if tmp_path:
                try:
                    os.unlink(tmp_path)
                    logger.debug("[Telegram] Removed temp voice file %s", tmp_path)
                except OSError:
                    logger.debug("[Telegram] Failed to remove temp voice file %s", tmp_path, exc_info=True)
